...
\textsc{nmf} is expected to be robust to various noises. Robust algorithms has to successfully reduce the amount of noise while preserving the edges and has no blurring effect on the image \citep{barbu2013variational}. Figure~\ref{noises} shows three kinds of noises we designed, including Gaussian noise, Poisson noise, and Salt \& Pepper noise. 
 
\textbf{Gaussian noise} is a noise with a probability density function being normal with mean zero. \textbf{Poisson noise} or shot noise is a type of electronic noise that
...
in bright regions and bright pixels in dark regions \citep{sampat2005computer}.
\begin{figure}\label{noises}
	\centering
	\includegraphics[scale=.8]{Noise_ORL_Normal_Comparison.png}\\ %replace these with jpg files with appropriate size
	\includegraphics[scale=.8]{Noise_ORL_Poisson_Comparison.png}\\
	\includegraphics[scale=.8]{Noise_ORL_Salt_and_Pepper_Comparison.png}
...
\end{figure}
\subsection{Multiple initial estimates underpin convexity}
...
We use the factorized matrices~$W$ and~$H$ corresponding to the least objective function~\eqref{eq:obnmf} and~\eqref{eq:klobj}, for \textsc{nmf} and~\textsc{klnmf}, algorithms respectively, as our final result of factorization.
This 
